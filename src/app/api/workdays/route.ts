...
export async function GET(request: NextRequest) {
  try {
    ...
    const workDaysWithCalculations = workDays.map(day => {
      // Calcular KM total das sessões de condução (correto para dupla de motoristas)
      let kmFromSessions = 0;
      if (day.drivingSessions && day.drivingSessions.length > 0) {
        kmFromSessions = day.drivingSessions.reduce((total, session) => {
      ...
      const kmTraveled = kmFromSessions > 0 
        ? kmFromSessions 
        : (day.endKm && day.startKm ? day.endKm - day.startKm : null);
      ...
      if (hoursWorked === null && day.startTime && day.endTime) {
      ...
      if (day.drivingSessions && day.drivingSessions.length > 0) {
        const lastSession = day.drivingSessions[day.drivingSessions.length - 1];
        lastSessionKm = lastSession.endKm || lastSession.startKm;
      }

      // Contar sessões de condução
      const sessionCount = day.drivingSessions?.length || 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const workDay = await db.workDay.create({
      data: {
        ...dataToSave,
        // Criar primeira sessão de condução
        drivingSessions: {
          create: {
            startTime: startTime || new Date().toTimeString().slice(0, 5),
            startKm: kmValue,
